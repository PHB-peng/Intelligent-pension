const [CHAR_TILDE, CODE_FNC1, SET_STARTA, SET_STARTB, SET_STARTC, SET_SHIFT, SET_CODEA, SET_CODEB, SET_STOP] = [126,
	102, 103, 104, 105, 98, 101, 100, 106
];
const REPLACE_CODES = {
...
module.exports = {
	...
	BarCode: function(options) {
		const ctx = uni.createCanvasContext(options.id);
		const width = parseInt(options.width);
		const height = parseInt(options.height);
		const codes = stringToCode128(options.code);
		const color = options.color || '#000000';
		const bgColor = options.bgColor || '#FFFFFF';
		const g = new Graphics(ctx, width, height, color, bgColor);
	...
	QRCode: function(options, ecc) {
		const adelta = [0, 11, 15, 19, 23, 27, 31, 16, 18, 20, 22, 24, 26, 28, 20, 22, 24, 24, 26, 28, 28, 22,
			24, 24, 26, 26, 28, 28, 24, 24, 26, 26, 26, 28, 28, 24, 26, 26, 26, 28, 28
		];
		const vpat = [0xc94, 0x5bc, 0xa99, 0x4d3, 0xbf6, 0x762, 0x847, 0x60d, 0x928, 0xb78, 0x45d, 0xa17, 0x532,
			0x9a6, 0x683, 0x8c9, 0x7ec, 0xec4, 0x1e1, 0xfab, 0x08e, 0xc1a, 0x33f, 0xd75, 0x250, 0x9d5,
			0x6f0, 0x8ba, 0x79f, 0xb0b, 0x42e, 0xa64, 0x541, 0xc69
		];
		const fmtword = [0x77c4, 0x72f3, 0x7daa, 0x789d, 0x662f, 0x6318, 0x6c41, 0x6976, 0x5412, 0x5125, 0x5e7c,
		...
		const eccblocks = [1, 0, 19, 7, 1, 0, 16, 10, 1, 0, 13, 13, 1, 0, 9, 17, 1, 0, 34, 10, 1, 0, 28, 16, 1,
		...
		const glog = [0xff, 0x00, 0x01, 0x19, 0x02, 0x32, 0x1a, 0xc6, 0x03, 0xdf, 0x33, 0xee, 0x1b, 0x68, 0xc7,
		...
		const gexp = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1d, 0x3a, 0x74, 0xe8, 0xcd, 0x87, 0x13,
		...
		const canvas = options.id || _canvas;
		if (!canvas) {
			console.warn('No canvas provided to draw QR code in!');
			return;
		}
		let size = options.size || _size || Math.min(canvas.width, canvas.height);
		const frame = genframe(options.code);
		const ctx = uni.createCanvasContext(options.id);
		...
		if (options.img) {
			try {
				const num = Number(((options.size - options.iconSize) / 2).toFixed(2));
				const [x, y] = [num, num];
				const [dWidth, dHeight] = [options.iconSize || 30, options.iconSize || 30];
...
let Graphics = function(ctx, width, height, color, bgColor) {
	this.width = width;
	this.height = height;
